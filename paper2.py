...
def process_submission(
        ...
        channel_type_value,
        virtual_subnet_rate_value, # This is an unused parameter in the original function. Check its purpose or remove if truly not needed. For now, keeping as is.
    ...
    # NEW: Initialize the new error message for virtual subnet
    virtual_subnet_error_md_update = gr.update(value="", visible=False)
    ### 结束新增
    ...
    if not student_name:
        ...
        return (download_file_output_update, check_result_md_output_update, analysis_output_md_update,
                ...
                network_analysis_error_md_update,
                virtual_subnet_error_md_update) # Updated return for early exit
    ...
    # NEW: Always show "频率规划错误" for virtual subnet
    virtual_subnet_error_md_update = gr.update(value="<span style='color: red; font-weight: bold;'>频率规划错误</span>", visible=True)
    ...
    return (download_file_output_update, check_result_md_output_update, analysis_output_md_update,
            ...
            final_eval_student_name_display_update,
            network_analysis_error_md_update,
            virtual_subnet_error_md_update) # NEW: Updated return
...
def submit_final_evaluation(
    ...
    final_scores.append(max(0, min(100, float(first_submission_avg_score))) if first_submission_avg_score is not None else 0)
...
with gr.Blocks(css=custom_css) as demo:
        ...
        with gr.Tab("任务内容", id="paper_app_tab"):
            ...
            with gr.Tabs() as main_tabs:
                with gr.Tab("答卷填写与分析", id="paper_tab"):
                    ...
                    # NEW: Add the specific error message for virtual subnet here
                    virtual_subnet_error_message = gr.Markdown("", visible=False, elem_classes=["error-message"])
                    gr.Markdown("---")
    ...
    submit_button.click(
        ...
        inputs=[
            ...
            gr.State(None), # Placeholder for virtual_subnet_rate_value which is not connected to a UI component
        ...
        outputs=[
            ...
            virtual_subnet_error_message # NEW: Added the new markdown output
        ]
    ).then(
